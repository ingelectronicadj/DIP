...
im1 = imread('C:\Users\Mc\Pictures\Huellas\diego1.bmp');
...
%%algoritmo para determinar centro de la huella
%%se recorrera la imagen 11, y aquellas pisciones donde halla un uno,
%%a¿haran que se rellene con un determinadno elemento estrcuturante en una
%%matrzi de mascara, para asi obtener un contorno
%%se dilata la imagen para hacerla mas parecidad a un poligono
se1=ones(10);
m1=imdilate(im10,se1);
figure(6), imshow(m1)
ss=regionprops(m1,'centroid');
centroids=cat(1, ss.Centroid);
plot(centroids(:,1),centroids(:,2),'b*')
figure(7), imshow(m1);
tam=length(centroids);%%se define el tamaño del vector que contiene las coordenadas de los centroides
for i=1:tam
    mx=uint16(centroids(i,1));
    my=uint16(centroids(i,2));
    if(mx>121)&&(my>107)&&(mx<227)&&(my<285)
        cx=mx;
        cy=my;
    end
end
